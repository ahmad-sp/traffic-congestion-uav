                  ...
                  return (
                    <button
                      ...
                      className={`w-full mt-2 px-3 py-2 rounded flex items-center justify-between text-sm transition
                        ${isSelected ? 'bg-blue-500/20 border border-blue-500/50' : 'bg-gray-700/50 hover:bg-gray-700'}`}
                    >
                      <div className="flex items-center gap-2">
                        <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${BADGE_STYLES[level]}`}>
                          {level}
                        </span>
                        <span>{arm.name}</span>
                      </div>
                      <div className="flex items-center gap-4 text-gray-400">
                        {status && (
                          <>
                            <span>VPM: <span className="text-white">{status.VPM ?? '–'}</span></span>
                            <span>Queue: <span className="text-white">{status.queue_depth ?? '–'}</span></span>
                          </>
                        )}
                      </div>
                    </button>
